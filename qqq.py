...
default_marker_length = 0.025  # 기본값(사용하지 않음)

########################################
# ArUco 설정
########################################

# ArUco 사전 및 파라미터 (DICT_4X4_50 사용)
aruco_dict = cv2.aruco.getPredefinedDictionary(cv2.aruco.DICT_4X4_50)
aruco_params = cv2.aruco.DetectorParameters()  # 클래스 생성자로 사용
detector_obj = cv2.aruco.ArucoDetector(aruco_dict, aruco_params)

########################################
# 사용자 정의: 단일 마커 포즈 추정 함수
########################################

def estimate_pose_single_marker(corners, marker_length, camera_matrix, dist_coeffs):
    """
    주어진 코너 (shape: (1,4,2))와 마커 크기(marker_length, m)를 이용하여,
    cv2.solvePnP를 통해 포즈(rvec, tvec)를 추정합니다.
    """
...
def compute_relative_positions(pose_dict, base_tag=1, target_tags=[2,3,4,5]):
    t0 = time.time()
    relative_positions = {}
...
def main():
    ...
    cap.set(cv2.CAP_PROP_FPS, 15)  # 15 FPS 설정

    fourcc_val = int(cap.get(cv2.CAP_PROP_FOURCC))
    width = cap.get(cv2.CAP_PROP_FRAME_WIDTH)
    height = cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
    fps = cap.get(cv2.CAP_PROP_FPS)
    print(f"현재 적용된 코덱: {chr(fourcc_val & 0xFF)}{chr((fourcc_val >> 8) & 0xFF)}"
          f"{chr((fourcc_val >> 16) & 0xFF)}{chr((fourcc_val >> 24) & 0xFF)}")
    print(f"카메라 해상도: {width} x {height} / {fps}")

    if not cap.isOpened():
        print("카메라 열기 실패")
        return
    print(f"카메라 열림: 인덱스 {cam_index}")

    frame_count = 0
    last_roi = None
    prev_frame_time = None

    print("ArUco 마커 검출 및 1번 마커 기준 상대 좌표 계산 시작 (ESC 종료)")
    while True:
        ...
        # 매 15프레임마다 ROI 업데이트, 그 외에는 이전 ROI 재사용
        if frame_count % 15 == 0:
        ...
        process_time = (time.time() - capture_timestamp) * 1000
        print(f"[{datetime.datetime.now()}] 디텍션 처리 완료 (처리 시간: {process_time:.2f} ms)")
        ...
        print(f"[{datetime.datetime.now()}] 1번 마커 업데이트: {update_time:.2f} ms")

        rel_start = time.time()
        TARGET_TAGS = [2,3,4,5]
        rel_left = compute_relative_positions(pose_dict, base_tag=BASE_TAG, target_tags=TARGET_TAGS)
        rel_time = (time.time() - rel_start) * 1000
        print(f"[{datetime.datetime.now()}] 상대 좌표 계산: {rel_time:.2f} ms")

        if prev_frame_time is not None:
            delta_time = (capture_timestamp - prev_frame_time) * 1000
            print(f"[{datetime.datetime.now()}] 프레임 간 시간 차이: {delta_time:.2f} ms")
        prev_frame_time = capture_timestamp

        overall_time = (time.time() - overall_start) * 1000
        print(f"[{datetime.datetime.now()}] 전체 한 프레임 처리 (이전 프레임 간 시간): {overall_time:.2f} ms\n")
